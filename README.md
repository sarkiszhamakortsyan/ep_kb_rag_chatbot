...
- [API](#api)
- [Extra features (dev-features branch)](#extra-features-dev-features-branch)
...
| `GET /admin/session` | Admin only (`Authorization: Bearer <ADMIN_TOKEN>`): checks the token |
| `GET /admin/history` | Admin only: stored questions and answers, newest first. Filters `q`, `provider`, `refused`, `from`, `to`; paging `limit`, `offset` |
| `GET /admin/history/{message_id}` | Admin only: one turn with the full answer, sources, tokens and timings |
| `GET /admin/history/export.csv` | Admin only: the filtered history as CSV |
| `GET /admin/stats` | Admin only: usage statistics for `from`–`to` (default: the last 30 days) |
| `GET /admin/costs` | Admin only: estimated costs per day and model, a what-if with other Claude models, and optimisation hints |
| `POST /admin/costs/advice` | Admin only: asks Claude for cost advice on the aggregated figures (no questions or answers are sent) |
...
## Extra features (dev-features branch)
...
frontend/src/
  ...
  features/admin/   admin area: sign-in, Statistics, Costs and History tabs, answer details
documentation/
...
- **Planned** (with the seams already in the code, see [`ideas.md`](documentation/taskdocs/ideas.md)): a tests tab; concise vs detailed answers; a model on/off toggle. The response history, statistics and costs are built on this branch.
