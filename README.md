...
backend/
  app/
    ...
    stores/         vector/ (in-memory numpy store), events/ (per-turn hook), history/ (SQLite + migrations)
...
frontend/src/
  ...
  features/admin/   admin area: sign-in, History tab, answer details
documentation/
...
- **Planned** (with the seams already in the code, see [`ideas.md`](documentation/taskdocs/ideas.md)): statistics, costs and tests tabs; concise vs detailed answers; a model on/off toggle. The response history is built on this branch.
