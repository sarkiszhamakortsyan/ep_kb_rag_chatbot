...
type Props = { onSignIn: (token: string, session: AdminSession) => void; checking: boolean };
...
function describe(err: unknown): string {
  ...
  if (err instanceof ApiError && err.code === "unauthorized") return "That token is not valid.";
  ...
  return (
    <div className="flex min-h-dvh items-center justify-center bg-canvas px-4">
      ...
      >
        ...
        <p className="mt-1 text-sm text-ink-muted">Enter the admin token configured on the server.</p>
        <label className="mt-5 block text-sm font-medium text-ink" htmlFor="admin-token">
          Admin token
        </label>
        <input
          id="admin-token"
        ...
        <a href="/" className="mt-4 block text-center text-sm text-ink-muted underline-offset-2 hover:underline">
          Back to the chat
        </a>
      </form>
    </div>
